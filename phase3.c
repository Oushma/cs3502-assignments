...
// Global flag to detect if threads are still running
...
// DEADLOCK-PRONE THREAD FUNCTION
void* teller_thread(void* arg) {
    ...
    // - Even tellers (0, 2): Transfer from Account 0 to Account 1
    // - Odd tellers (1, 3): Transfer from Account 1 to Account 0
    int from_id, to_id;
    if (teller_id % 2 == 0) {
        from_id = 0;
        to_id = 1;
...
int main() {
    ...
    printf("  Both lock FROM account first, then TO account\n");
    printf("  This creates circular wait -> DEADLOCK!\n\n");
    ...
    // Since we can't use pthread_tryjoin, we'll just join and see if it hangs
    printf("Waiting for all threads to complete...\n");
    printf("(If deadlocked, this program will hang forever)\n");
    printf("(Press Ctrl+C to kill if it hangs)\n\n");
    
    // Try to join threads - this will hang if deadlocked
    for (int i = 0; i < NUM_THREADS; i++) {
    ...
    // If we get here, all threads completed (no deadlock)
    printf("\nAll threads completed successfully (no deadlock in this run).\n");
    ...
    if (total_actual == total_expected) {
        ...
        printf("   Deadlocks are timing-dependent!\n");
        printf("   Try running again with different delays or more threads.\n");
    } else {
        printf("\nBALANCE MISMATCH: Some transactions may not have completed.\n");
        printf("   This may indicate a deadlock occurred.\n");
    }
